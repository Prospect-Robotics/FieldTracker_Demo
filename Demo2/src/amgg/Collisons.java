...
class Collisons {
	...
	static void makeMovementHitbox(Point.Double destination) {
		...
		for(Point.Double point:new Point.Double[] {tl,tr}){
			PointMath.moveAlong(point, FieldTracker.getDistanceToPoint(destination), FieldTracker.getAngleToPoint(destination));
		}
